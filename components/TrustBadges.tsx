...
/* ── Logo SVG de Mercado Pago (apretón de manos oficial de Wikimedia) ── */
export function MercadoPagoLogo({ height = 36, variant = 'color' }: { height?: number; variant?: 'color' | 'light' }) {
  const ovalBg = '#00bcff';
  const handshakeBg = '#ffffff';
  const textBg = variant === 'light' ? '#ffffff' : '#0a0080'; // azul oscuro oficial
  ...
  if (variant === 'compact') {
    return (
      <div style={{
        ...
        <span style={{ fontSize: '0.75rem', color: textColor, fontWeight: '700' }}>
          Compra segura con
        </span>
        <MercadoPagoLogo height={28} />
      </div>
    );
  }
